...
public class Hangman {
  ...
  /**
   * A simple game where you guess try to guess a random word (hangman). This is the most developed
   * class of my project.
   */
      ...
      while (lives >= 1) {
        ...
        if (guess.equalsIgnoreCase(hint)) {
          ...
          break; // Quits the loop when user guesses the correct word
            ...
            || guess.equalsIgnoreCase(letterArray[4]) || guess.equalsIgnoreCase(letterArray[5])) { 
          String underscore = "";
  ...
  /**
   * Returns the number that will randomly choose a new word.
   * @return restartNum
   *  
   */
  ...
  /**
   * Returns the boolean playAgain depending on if the user chooses to continue or stop.
   * @param playAgain 
   * @param input 
   * @param guess
   * @return playAgain
